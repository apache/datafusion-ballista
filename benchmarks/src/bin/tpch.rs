...
enum TpchOpt {
    Benchmark(BallistaBenchmarkOpt),
    Convert(ConvertOpt),
...
async fn benchmark_ballista(opt: BallistaBenchmarkOpt) -> Result<Vec<RecordBatch>> {
...
mod ci {
    ...
    use datafusion::logical_expr::{Expr, Cast, trim, col};
    use ballista::prelude::Result;
    use ballista_core::error::BallistaError;
    use crate::{assert_expected_results, BallistaBenchmarkOpt, benchmark_ballista, get_answer_schema, string_schema};
    ...
    async fn verify_query(n: usize) -> Result<()> {
        ...
        let actual = benchmark_ballista(opt).await?;
