...
/// This operator sends a logical plan to a kapot scheduler for execution and
...
async fn execute_query(
    ...
    info!("Connecting to kapot scheduler at {}", scheduler_url);
    // TODO reuse the scheduler to avoid connecting to the kapot scheduler again and again
...
async fn fetch_partition(
    ...
    let mut kapot_client = KapotClient::try_new(host, port)
        .await
        .map_err(|e| DataFusionError::Execution(format!("{e:?}")))?;
    kapot_client
        .fetch_partition(
