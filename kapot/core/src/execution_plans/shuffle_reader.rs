...
struct AbortableReceiverStream {
    inner: ReceiverStream<result::Result<SendableRecordBatchStream, KapotError>>,
...
impl AbortableReceiverStream {
    ...
    pub fn create(
        rx: tokio::sync::mpsc::Receiver<
            result::Result<SendableRecordBatchStream, KapotError>,
        >,
...
impl PartitionReader for PartitionReaderEnum {
    // Notice return `kapotError::FetchFailed` will let scheduler re-schedule the task.
    async fn fetch_partition(
        &self,
        location: &PartitionLocation,
    ) -> result::Result<SendableRecordBatchStream, KapotError> {
...
async fn fetch_partition_remote(
    ...
    let mut kapot_client =
        KapotClient::try_new(host, port)
            .await
            .map_err(|error| match error {
                // map grpc connection error to partition fetch error.
                KapotError::GrpcConnectionError(msg) => KapotError::FetchFailed(
                    metadata.id.clone(),
...
async fn fetch_partition_local(
    ...
    let reader = fetch_partition_local_inner(path).map_err(|e| {
        // return kapotError::FetchFailed may let scheduler retry this task.
...
fn fetch_partition_local_inner(
    path: &str,
) -> result::Result<StreamReader<BufReader<File>>, KapotError> {
    
    let file = File::open(path).map_err(|e| {
        KapotError::General(format!("Failed to open partition file at {path}: {e:?}"))
    })?;

    let buff_reader = std::io::BufReader::new(file);

    let reader = StreamReader::try_new(buff_reader, None).map_err(|e| {
        KapotError::General(format!("Failed to new arrow FileReader at {path}: {e:?}"))
    })?;
...
mod tests {
    ...
    async fn test_fetch_partitions_error_mapping() -> Result<()> {
        ...
        // kapotError::FetchFailed -> ArrowError::ExternalError -> kapotError::FetchFailed
        let kapot_error = batches.unwrap_err();
        assert!(matches!(
            kapot_error,
            KapotError::FetchFailed(_, _, _, _)
        ));
