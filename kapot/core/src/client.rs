...
/// Client for interacting with kapot executors.
#[derive(Clone)]
pub struct KapotClient {
...
impl KapotClient {
    /// Create a new kapotClient to connect to the executor listening on the specified
    /// host and port
    pub async fn try_new(host: &str, port: u16) -> Result<Self> {
        let addr = format!("http://{host}:{port}");
        debug!("kapotClient connecting to {}", addr);
        let connection =
            create_grpc_client_connection(addr.clone())
                .await
                .map_err(|e| {
                    KapotError::GrpcConnectionError(format!(
                    "Error connecting to kapot scheduler or executor at {addr}: {e:?}"
                ))
                })?;
        let flight_client = FlightServiceClient::new(connection);
        debug!("kapotClient connected OK");
    ...
    pub async fn fetch_partition(
        ...
        self.execute_action(&action)
            ...
            .map_err(|error| match error {
                ...
                KapotError::GrpcActionError(msg) => KapotError::FetchFailed(
    ...
    pub async fn execute_action(
        ...
        serialized_action
            .encode(&mut buf)
            .map_err(|e| KapotError::GrpcActionError(format!("{e:?}")))?;
            ...
            let res = match result {
                ...
                Err(ref err) => {
                    ...
                    if i == IO_RETRIES_TIMES - 1 || err.code() != Code::Unknown {
                        return KapotError::GrpcActionError(format!(
                            "{:?}",
                            result.unwrap_err()
                        ))
                        .into();
            ...
            match stream.message().await {
                Ok(res) => {
                    return match res {
                        ...
                        None => Err(KapotError::GrpcActionError(
