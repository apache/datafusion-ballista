...
use crate::serde::{protobuf, KapotCodec};
...
) -> Result<TaskDefinition, KapotError> {
...
>(
    ...
    window_functions: HashMap<String, Arc<WindowUDF>>,
    codec: KapotCodec<T, U>,
) -> Result<Vec<TaskDefinition>, KapotError> {
    let mut props = HashMap::new();
...
fn reset_metrics_for_execution_plan(
    ...
    .data()
    .map_err(KapotError::DataFusionError)
}
