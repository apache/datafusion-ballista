...
//! This crate contains code generated from the kapot Protocol Buffer Definition as well
//! as convenience code for interacting with the generated code.

use crate::{error::KapotError, serde::scheduler::Action as kapotAction};
...
use crate::serde::protobuf::kapot_physical_plan_node::PhysicalPlanType;
use crate::serde::scheduler::PartitionLocation;
pub use generated::kapot as protobuf;
...
pub fn decode_protobuf(bytes: &[u8]) -> Result<kapotAction, KapotError> {
    let mut buf = Cursor::new(bytes);

    protobuf::Action::decode(&mut buf)
        .map_err(|e| KapotError::Internal(format!("{e:?}")))
        .and_then(|node| node.try_into())
}

#[derive(Clone, Debug)]
pub struct KapotCodec<
...
impl Default for KapotCodec {
    fn default() -> Self {
        Self {
            logical_extension_codec: Arc::new(KapotLogicalExtensionCodec::default()),
            physical_extension_codec: Arc::new(KapotPhysicalExtensionCodec {}),
...
impl<T: 'static + AsLogicalPlan, U: 'static + AsExecutionPlan> KapotCodec<T, U> {
    pub fn new(
        logical_extension_codec: Arc<dyn LogicalExtensionCodec>,
        physical_extension_codec: Arc<dyn PhysicalExtensionCodec>,
    ) -> Self {
...
impl Default for KapotLogicalExtensionCodec {
    fn default() -> Self {
        Self {
...
impl LogicalExtensionCodec for KapotLogicalExtensionCodec {
    fn try_decode(
        &self,
        buf: &[u8],
...
#[derive(Debug)]
pub struct KapotPhysicalExtensionCodec {}

impl PhysicalExtensionCodec for KapotPhysicalExtensionCodec {
    fn try_decode(
        &self,
        buf: &[u8],
        ...
        let kapot_plan: protobuf::KapotPhysicalPlanNode =
            protobuf::KapotPhysicalPlanNode::decode(buf).map_err(|e| {
                DataFusionError::Internal(format!(
                    "Could not deserialize kapotPhysicalPlanNode: {e}"
                ))
            })?;

        let kapot_plan =
            kapot_plan.physical_plan_type.as_ref().ok_or_else(|| {
                DataFusionError::Internal(
                    "Could not deserialize kapotPhysicalPlanNode because it's physical_plan_type is none".to_string()
                )
            })?;
    ...
    fn try_encode(
        ...
        if let Some(exec) = node.as_any().downcast_ref::<ShuffleWriterExec>() {
            ...
            let proto = protobuf::KapotPhysicalPlanNode {
