...
use kapot_core::error::{kapot_error, KapotError, Result};
...
impl SledClient {
    ...
    pub fn try_new<P: AsRef<std::path::Path>>(path: P) -> Result<Self> {
        Ok(Self {
            db: sled::open(path).map_err(sled_to_kapot_error)?,
    ...
    pub fn try_new_temporary() -> Result<Self> {
        Ok(Self {
            db: sled::Config::new()
                .temporary(true)
                .open()
                .map_err(sled_to_kapot_error)?,
            locks: Arc::new(Mutex::new(HashMap::new())),
        })
    }
}

fn sled_to_kapot_error(e: sled::Error) -> KapotError {
    match e {
        sled::Error::Io(io) => KapotError::IoError(io),
        _ => KapotError::General(format!("{e}")),
    }
}
...
impl KeyValueStore for SledClient {
    async fn get(&self, keyspace: Keyspace, key: &str) -> Result<Vec<u8>> {
        ...
        Ok(self
            .db
            .get(key)
            .map_err(|e| kapot_error(&format!("sled error {e:?}")))?
            .map(|v| v.to_vec())
            .unwrap_or_default())
    }
    ...
    async fn put(&self, keyspace: Keyspace, key: String, value: Vec<u8>) -> Result<()> {
        ...
        self.db
            ...
            .map_err(|e| {
                ...
                kapot_error("sled insert failed")
            })
            .map(|_| ())
    }
    ...
    async fn apply_txn(&self, ops: Vec<(Operation, Keyspace, String)>) -> Result<()> {
        ...
        self.db.apply_batch(batch).map_err(|e| {
            ...
            kapot_error("sled operations failed")
        })
    }
    ...
    async fn delete(&self, keyspace: Keyspace, key: &str) -> Result<()> {
        ...
        self.db.remove(key).map_err(|e| {
            ...
            kapot_error("sled delete failed")
        })?;
        Ok(())
    }
}
