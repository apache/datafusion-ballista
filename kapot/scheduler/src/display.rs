...
//! Implementation of kapot physical plan display with metrics. See
//! [`crate::physical_plan::displayable`] for examples of how to
//! format

use kapot_core::utils::collect_plan_metrics;
...
pub fn print_stage_metrics(
    ...
    if plan_metrics.len() == stage_metrics.len() {
        ...
        info!(
            ...
            job_id,
            stage_id,
            DisplayablekapotExecutionPlan::new(plan, &plan_metrics).indent()
        );
    } else {
...
pub struct DisplayablekapotExecutionPlan<'a> {
    inner: &'a dyn ExecutionPlan,
    metrics: &'a Vec<MetricsSet>,
}

impl<'a> DisplayablekapotExecutionPlan<'a> {
...
impl<'a> ToStringifiedPlan for DisplayablekapotExecutionPlan<'a> {
    fn to_stringified(
        &self,
