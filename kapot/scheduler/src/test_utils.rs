...
use kapot_core::error::{KapotError, Result};
...
use kapot_core::config::{KapotConfig, KAPOT_DEFAULT_SHUFFLE_PARTITIONS};
use kapot_core::serde::protobuf::job_status::Status;
...
use kapot_core::serde::scheduler::{
    ExecutorData, ExecutorMetadata, ExecutorSpecification,
};
use kapot_core::serde::{protobuf, KapotCodec};
...
use kapot_core::utils::default_session_builder;
...
pub fn test_cluster_context() -> KapotCluster {
    KapotCluster::new_memory(TEST_SCHEDULER_NAME, default_session_builder)
}
...
impl TaskLauncher for VirtualTaskLauncher {
    async fn launch_tasks(
        ...
        self.sender
            ...
            .await
            .map_err(|e| {
                KapotError::Internal(format!("Error sending task status: {e:?}"))
            })
    }
}
...
impl SchedulerTest {
    pub async fn new(
        ...
        let kapot_config = if num_executors > 0 && task_slots_per_executor > 0 {
            KapotConfig::builder()
                .set(
                    KAPOT_DEFAULT_SHUFFLE_PARTITIONS,
                    format!("{}", num_executors * task_slots_per_executor).as_str(),
                )
                .build()?
        } else {
            KapotConfig::builder().build()?
        };
    ...
    pub async fn ctx(&self) -> Result<Arc<SessionContext>> {
        self.scheduler
            .state
            .session_manager
            .create_session(&self.kapot_config)
            .await
    }
    ...
    pub async fn tick(&mut self) -> Result<()> {
        if let Some(receiver) = self.status_receiver.as_mut() {
            if let Some((executor_id, status)) = receiver.recv().await {
                ...
                return Err(KapotError::Internal("Task sender dropped".to_owned()));
            }
        } else {
            return Err(KapotError::Internal(
