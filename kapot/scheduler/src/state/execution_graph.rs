...
use kapot_core::execution_plans::{ShuffleWriterExec, UnresolvedShuffleExec};
use kapot_core::serde::protobuf::failed_task::FailedReason;
...
use kapot_core::serde::protobuf::{job_status, FailedJob, ShuffleWritePartition};
use kapot_core::serde::protobuf::{task_status, RunningTask};
    ...
    pub fn update_task_status(
        ...
        for (stage_id, stage_task_statuses) in job_task_statuses {
            if let Some(stage) = self.stages.get_mut(&stage_id) {
                ...
                return Err(KapotError::Internal(format!(
        ...
        let mut next_task = self.stages.iter_mut().find(|(_stage_id, stage)| {
            ...
            if let ExecutionStage::Running(stage) = stage {
                let (partition_id, _) = stage
                    ...
                    .ok_or_else(|| {
                        KapotError::Internal(format!("Error getting next task for job {job_id}: Stage {stage_id} is ready but has no pending tasks"))
                    })?;
                ...
                Err(KapotError::General(format!("Stage {stage_id} is not a running stage")))
            }
        }).transpose()?;
...
/// Represents the basic unit of work for the kapot executor. Will execute
