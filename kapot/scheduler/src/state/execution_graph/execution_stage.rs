...
use kapot_core::serde::scheduler::PartitionLocation;
use kapot_core::serde::KapotCodec;
use datafusion_proto::physical_plan::AsExecutionPlan;

use crate::display::DisplayablekapotExecutionPlan;
...
impl UnresolvedStage {
    ...
    pub(super) fn add_input_partitions(
        ...
        if let Some(stage_inputs) = self.inputs.get_mut(&stage_id) {
            ...
            return Err(KapotError::Internal(format!("Error adding input partitions to stage {}, {} is not a valid child stage ID", self.stage_id, stage_id)));
        }
    ...
    pub(super) fn remove_input_partitions(
        ...
        if let Some(stage_output) = self.inputs.get_mut(&input_stage_id) {
            ...
            Err(KapotError::Internal(format!("Error remove input partition for Stage {}, {} is not a valid child stage ID", self.stage_id, input_stage_id)))
        }
    }
    ...
    pub(super) fn update_task_metrics(
        ...
        let new_metrics_set = if let Some(combined_metrics) = &mut self.stage_metrics {
            if metrics.len() != combined_metrics.len() {
                return Err(KapotError::Internal(format!("Error updating task metrics to stage {}, task metrics array size {} does not equal \
...
impl Debug for SuccessfulStage {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let plan = DisplayablekapotExecutionPlan::new(
            self.plan.as_ref(),
            &self.stage_metrics,
        )
        .indent();
