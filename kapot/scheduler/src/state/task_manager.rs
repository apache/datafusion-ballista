        ...
        for job_id in &job_ids {
            if let Some(cached) = self.get_active_execution_graph(job_id) {
                ...
                let graph = self.state
                    ...
                    .await?
                    .ok_or_else(|| KapotError::Internal(format!("Error getting job overview, no execution graph found for job {job_id}")))?;
        ...
        if let Some(mut job_info) = self.active_job_cache.get_mut(&job_id) {
            ...
            if task.data_cache {
                props.push(KeyValuePair {
                    key: KAPOT_DATA_CACHE_ENABLED.to_string(),
    ...
    fn prepare_multi_task_definition(
        ...
        if let Some(task) = tasks.first() {
            ...
            if let Some(mut job_info) = self.active_job_cache.get_mut(&job_id) {
                ...
                Err(KapotError::General(format!("Cannot prepare multi task definition for job {job_id} which is not in active cache")))
            }
        } else {
