...
impl DistributedPlanner {
    ...
    fn plan_query_stages_internal<'a>(
        ...
        {
            Err(KapotError::NotImplemented(format!(
...
mod test {
    ...
    use kapot_core::execution_plans::UnresolvedShuffleExec;
    ...
    #[tokio::test]
    async fn distributed_aggregate_plan() -> Result<(), KapotError> {
        ...
        ShuffleWriterExec: Some(Hash([Column { name: "l_returnflag", index: 0 }], 2))
          AggregateExec: mode=Partial, gby=[l_returnflag@1 as l_returnflag], aggr=[SUM(lineitem.l_extendedprice * Int64(1))]
            CsvExec: files={2 groups: [[kapot/scheduler/testdata/lineitem/partition1.tbl], [kapot/scheduler/testdata/lineitem/partition0.tbl]]}, has_header=false, limit=None, projection=[l_extendedprice, l_returnflag]
    ...
    #[tokio::test]
    async fn distributed_join_plan() -> Result<(), KapotError> {
        let ctx = datafusion_test_context("testdata").await?;
        let session_state = ctx.state();
...
",
    ...
    #[tokio::test]
    async fn roundtrip_serde_aggregate() -> Result<(), KapotError> {
        let ctx = datafusion_test_context("testdata").await?;
        let session_state = ctx.state();
    ...
    fn roundtrip_operator(
        ctx: &SessionContext,
        plan: Arc<dyn ExecutionPlan>,
    ) -> Result<Arc<dyn ExecutionPlan>, KapotError> {
        let codec: KapotCodec<LogicalPlanNode, PhysicalPlanNode> =
            KapotCodec::default();
