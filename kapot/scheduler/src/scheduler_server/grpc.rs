...
use kapot_core::config::{KapotConfig, KAPOT_JOB_NAME};
use kapot_core::serde::protobuf::execute_query_params::{OptionalSessionId, Query};
use std::collections::HashMap;
use std::convert::TryInto;
use std::net::SocketAddr;

use kapot_core::serde::protobuf::executor_registration::OptionalHost;
use kapot_core::serde::protobuf::scheduler_grpc_server::SchedulerGrpc;
...
use kapot_core::serde::scheduler::ExecutorMetadata;
    ...
    for SchedulerServer<T, U>
{
    ...
    async fn create_session(
        ...
        // parse config
        let mut config_builder = KapotConfig::builder();
        ...
        } = query_params
        {
            ...
            let (session_id, session_ctx) = match optional_session_id {
                ...
                _ => {
                    ...
                    let config = KapotConfig::builder().build().map_err(|e| {
                        let msg = format!("Could not parse configs: {e}");
                        error!("{}", msg);
                        Status::internal(msg)
                    })?;
...
mod test {
    ...
    use kapot_core::serde::scheduler::ExecutorSpecification;
    ...
    #[tokio::test]
    async fn test_poll_work() -> Result<(), KapotError> {
    ...
    #[tokio::test]
    async fn test_stop_executor() -> Result<(), KapotError> {
        let cluster = test_cluster_context();
    ...
    #[tokio::test]
    async fn test_register_executor_in_heartbeat_service() -> Result<(), KapotError> {
        let cluster = test_cluster_context();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_expired_executor() -> Result<(), KapotError> {
        let cluster = test_cluster_context();
