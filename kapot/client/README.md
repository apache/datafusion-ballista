...
# kapot: Distributed Scheduler for Apache Arrow DataFusion

kapot is a distributed compute platform primarily implemented in Rust, and powered by Apache Arrow and
...
The foundational technologies in kapot are:
...
kapot can be deployed as a standalone cluster and also supports [Kubernetes](https://kubernetes.io/). In either
...
There are numerous ways to start a kapot cluster, including support for Docker and
Kubernetes. For full documentation, refer to the deployment section of the
[kapot User Guide](https://datafusion.apache.org/kapot/user-guide/deployment/)
...
```bash
RUST_LOG=info kapot-executor -c 4
```
...
```bash
RUST_LOG=info kapot-executor --bind-port 50052 -c 4
```

## Executing a query

kapot provides a `kapotContext` as a starting point for creating queries. DataFrames can be created
by invoking the `read_csv`, `read_parquet`, and `sql` methods.

To build a simple kapot example, run the following command to add the dependencies to your `Cargo.toml` file:

```bash
cargo add kapot datafusion tokio
```
