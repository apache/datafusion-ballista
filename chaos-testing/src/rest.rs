...
/// One shared client (and connection pool) for all polling. The tight polling
/// loops below would otherwise build a fresh `Client` per request — ~1200 new
/// connections through the k8s port-forward over a single 60s wait.
...
pub(crate) async fn running_job_id(rest_url: &str) -> Result<String, String> {
    ...
    // Retry on request errors until the deadline (a dropped/reconnecting k8s
    // port-forward makes single requests fail transiently), carrying the last
    // error into the timeout message.
    let mut last_err = "no request attempted".to_string();
    loop {
        match get_json(format!("{rest_url}/api/jobs")).await {
            Ok(body) => {
        ...
        if Instant::now() > deadline {
            return Err(format!(
                "timed out waiting for a job to appear; last error: {last_err}"
            ));
        }
...
pub(crate) async fn await_any_stage_running(
    ...
    // Retry on request errors until the deadline (see `running_job_id`).
    let mut last_err = "no request attempted".to_string();
    loop {
        let stages = match stages(rest_url, job_id).await {
            Ok(stages) => stages,
            Err(e) => {
                last_err = e;
                if Instant::now() > deadline {
                    return Err(format!(
                        "timed out waiting for any stage to start running; last error: {last_err}"
                    ));
                }
                tokio::time::sleep(Duration::from_millis(50)).await;
                continue;
