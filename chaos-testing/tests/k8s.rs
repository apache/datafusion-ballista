...
impl K8sRun {
    ...
    /// `terminationGracePeriodSeconds`. Scenario G uses a short grace so the kill
    /// is abrupt (the kubelet SIGKILLs the executors before they can drain).
    async fn start_with_executor_grace(
        aqe: bool,
        executors: usize,
...
///
/// The kill scales the executor Deployment to 0 (dropping the ReplicaSet's desired
/// count so it will not reschedule — unlike `--cascade=orphan`, which leaves it
/// recreating pods and the job recovers) then force-deletes the pods. k8s always
/// SIGTERMs before SIGKILL; the abruptness comes from the pods' short grace
/// (`ABRUPT_EXECUTOR_GRACE_SECONDS`), which SIGKILLs the executor before it can
/// drain the in-flight query. With no executor left and none rescheduled, the job
/// fails after the bounded no-executors grace rather than hanging.
///
/// The per-batch `chaos_delay` just has to outlast that grace so a task is still
/// running when the SIGKILL lands (otherwise the executor drains and the query
/// succeeds — how an earlier version, with the default 30s grace, spuriously
/// passed).
///
...
async fn killing_every_executor_terminates_the_job_on_k8s(#[case] aqe: bool) {
    ...
    // Short executor grace so the kill is abrupt: the kubelet SIGKILLs the
    // executors before their graceful path can drain the in-flight query.
    let run = K8sRun::start_with_executor_grace(
        aqe,
        2,
    ...
    // lands (> the pods' terminationGracePeriodSeconds); see the doc comment.
    let sql = Fixture::chaos_query("chaos_delay(f.key >= 0, 10000)");
    ...
    let job_id = match run.cluster.running_job_id().await {
        Ok(id) => id,
        Err(e) => {
            fail_with_diagnostics(&run.cluster, format!("job must appear: {e}")).await
        }
    };
    if let Err(e) = run.cluster.await_any_stage_running(&job_id).await {
        fail_with_diagnostics(
            &run.cluster,
            format!(
                "the job must start running a task before we remove its executors: {e}"
            ),
        )
        .await;
    }

    // Total loss that stays lost and is not drained: scale to 0 so no replacement
    // is scheduled, then force-kill the pods.
    run.cluster
        .kill_all_executors_hard()
        .await
        .expect("force-kill every executor");

    // Dump diagnostics on timeout — this is the assertion the scenario exists for,
    // and the namespace is dropped immediately after, so a CI hang leaves nothing.
    let result = match tokio::time::timeout(Duration::from_secs(120), query).await {
        Ok(joined) => joined.expect("query task should not panic"),
        Err(_) => {
            fail_with_diagnostics(
                &run.cluster,
                "job must terminate, not hang, after every executor is lost".to_string(),
            )
            .await
        }
    };
...
/// Dump cluster diagnostics, then panic. Used at scenario G's failure points so a
/// CI failure leaves the pod state and logs behind before the namespace is torn
/// down on drop.
async fn fail_with_diagnostics(cluster: &K8sCluster, msg: String) -> ! {
...
async fn restarted_executor_rejoins_and_serves_queries_on_k8s() {
    ...
    let after = await_replacement(&run.cluster, &before, before.len()).await;
...
/// Poll until the scheduler lists exactly `expected` executors and at least one
/// is not in `before` — i.e. the killed executor has been reaped and its
/// rescheduled replacement (new id) has registered.
