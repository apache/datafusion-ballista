...
fn restrict(
    ...
    // RangeFilterExec: cuts describe the K global partitions and are shared
    // across all restricted views; only the operator's `partition_indices`
    // mapping changes. We restrict the child first, then hand it +
    // `partitions` (task-local → global mapping) to the operator, which
    // re-derives its new mapping.
    if !under_collect && let Some(rf) = plan.downcast_ref::<RangeFilterExec>() {
        ...
        return Ok(Arc::new(rf.restrict_partitions(new_child, partitions)?));
    }
...
mod tests {
    ...
    /// A `RangeFilterExec` restricted to a subset of partitions must remap
    /// its `partition_indices` by the same indices in the same order as its
    /// child. Cuts stay whole (they describe the K global partitions);
    /// only the local→global mapping changes.
    #[test]
    fn range_filter_partition_indices_are_remapped_with_partitions() {
        ...
        assert_eq!(rf.partition_indices(), &[1, 3]);
        assert_eq!(rf.cuts().unwrap(), cuts);
