...
async fn should_split_plan_into_runnable_stages_internal() -> datafusion::error::Result<()>
{
    ...
    let stages = planner._runnable_stages()?.unwrap();
    ...
    ExchangeExec: partitioning=Hash([c@0], 2), plan_id=0, stage_id=0, stage_resolved=false
      AggregateExec: mode=Partial, gby=[c@2 as c], aggr=[min(t.a), max(t.b)]
        DataSourceExec: partitions=1, partition_sizes=[1]
    ");
    planner.resolve_stage(0, mock_partitions_with_statistics())?;
          ...
          CoalesceBatchesExec: target_batch_size=8192
            ExchangeExec: partitioning=Hash([c@0], 2), plan_id=0, stage_id=0, stage_resolved=true
              AggregateExec: mode=Partial, gby=[c@2 as c], aggr=[min(t.a), max(t.b)]
                DataSourceExec: partitions=1, partition_sizes=[1]
    ");
    planner.resolve_stage(1, mock_partitions_with_statistics())?;

    let stages = planner._runnable_stages()?;
...
async fn should_split_stages_resolve_right_branch() -> datafusion::error::Result<()> {
    ...
    let runnable_stages = planner._runnable_stages()?.unwrap();
    ...
    planner.resolve_stage(1, partition_locations.clone())?;
    ...
    planner.resolve_stage(0, partition_locations.clone())?;
    //
    ...
    planner.resolve_stage(2, partition_locations.clone())?;
    ...
    let runnable_stages = planner._runnable_stages()?.unwrap();
    assert_eq!(1, runnable_stages.len());
    planner.resolve_stage(3, partition_locations.clone())?;

    let runnable_stages = planner._runnable_stages()?;
    assert_eq!(1, runnable_stages.unwrap().len());

    planner.resolve_stage(4, partition_locations.clone())?;
    let runnable_stages = planner._runnable_stages()?;
