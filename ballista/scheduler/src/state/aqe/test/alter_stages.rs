...
use crate::state::aqe::AdaptivePlanner;
...
async fn should_propagate_empty_stage() -> datafusion::error::Result<()> {
    ...
    // empty output
    planner.resolve_stage(0, mock_partitions_with_statistics_no_data())?;
    ...
    ShuffleWriterExec: partitioning: None
      EmptyExec
    ");
    planner.resolve_stage(1, mock_partitions_with_statistics_no_data())?;
...
async fn should_insert_new_stage() -> datafusion::error::Result<()> {
    ...
    planner.resolve_stage(0, big_statistics_exchange())?;
    assert_plan!(planner.current_plan(),  @ r"
    AdaptiveDatafusionExec: is_final=false, plan_id=1, stage_id=pending
    ...
    planner.resolve_stage(1, small_statistics_exchange())?;
    let stages = planner.runnable_stages()?.unwrap();
    assert_eq!(1, stages.len());
    ...
    planner.resolve_stage(2, small_statistics_exchange())?;
    let stages = planner.runnable_stages()?;
    assert!(stages.is_none());
...
fn create_plan_scan() -> Result<Arc<dyn ExecutionPlan>, DataFusionError> {
    let (_, small) = create_big_and_small();

    let big_scan = big_scan(1);

    let column_small = Arc::new(Column::new_with_schema(
        "small_col",
        &Schema::new(vec![Field::new("small_col", DataType::Int32, false)]),
    )?);

    let exchange_small = Arc::new(RepartitionExec::try_new(
        small,
        Partitioning::Hash(vec![column_small], 2),
    )?);
    let join =
        Arc::new(CrossJoinExec::new(big_scan, exchange_small)) as Arc<dyn ExecutionPlan>;
    Ok(join)
}

fn create_plan_scan_() -> Result<Arc<dyn ExecutionPlan>, DataFusionError> {
...
fn create_plan() -> Result<Arc<dyn ExecutionPlan>, DataFusionError> {
    ...
    let join = Arc::new(CrossJoinExec::new(exchange_big, exchange_small))
