...
/// In some cases double [CoalesceBatchesExec] will be inserted.
/// Until we get chance to investigate the issue this rule will
/// eliminate doubles
#[derive(Debug, Clone, Default)]
pub struct EliminateCoalesceBatchesRule {}
/// for some reason double CoalesceBatchesExec
/// show
impl EliminateCoalesceBatchesRule {
    fn transform(
        execution_plan: Arc<dyn ExecutionPlan>,
    ) -> datafusion::error::Result<Transformed<Arc<dyn ExecutionPlan>>> {
...
impl PhysicalOptimizerRule for EliminateCoalesceBatchesRule {
    fn optimize(
        &self,
        execution_plan: Arc<dyn ExecutionPlan>,
        _config: &datafusion::config::ConfigOptions,
