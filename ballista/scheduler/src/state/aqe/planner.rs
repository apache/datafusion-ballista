...
impl AdaptivePlanner {
    ...
    ///
    /// Returns the set of stage_ids whose outputs were affected. The graph
    /// uses this set to (a) transition matching Successful stages back to
    /// Running, and (b) drop any Resolved/Running downstream stages whose
    /// embedded plan depends on the affected outputs (those will be
    /// regenerated by `actionable_stages` after the upstream stages re-run).
    pub(super) fn reset_on_lost_executor(
        &mut self,
        executor_id: &str,
    ) -> common::Result<HashSet<usize>> {
        ...
        let affected_ids: HashSet<usize> = affected.iter().map(|(id, _)| *id).collect();
...
mod tests {
    ...
    async fn reset_on_lost_executor_is_noop_when_no_locations_match() {
        ...
        // Force the planner to identify runnable stages so its caches get
        // populated.
        let _ = planner.runnable_stages().unwrap();

        // No exec node references "never-existed", so nothing should change.
