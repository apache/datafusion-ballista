...
pub(crate) struct AdaptiveExecutionGraph {
    ...
    stage_partitions: HashMap<usize, Vec<Vec<PartitionLocation>>>,
}
...
impl AdaptiveExecutionGraph {
    ...
    pub fn new(
        ...
        // TODO: make it try new
        let (runnable, _) = planner.actionable_stages()?;
        let runnable = runnable.unwrap();
        let stages: ballista_core::error::Result<HashMap<usize, ExecutionStage>> =
            runnable
                .into_iter()
                .map(|s| Self::convert_to_running_stage(session_config.clone(), s.plan))
                .collect();
        let stages = stages?;
        //let stages = stage_builder.build(runnable)?;
        ...
        Ok(Self {
            ...
            stage_partitions: HashMap::new(),
        })
    }
}

impl AdaptiveExecutionGraph {
    fn convert_to_running_stage(
        session_config: Arc<SessionConfig>,
    ...
    fn update_stage_output_links(
        ...
        _output_links: Vec<usize>,
    ) -> ballista_core::error::Result<Vec<usize>> {
        let resolved_stages = vec![];

        self.stage_partitions
            .entry(stage_id)
            .and_modify(|v| v.push(locations.clone()))
            .or_insert(vec![locations]);

        // TODO: we need to get links of input stages when stage is resolved
        //       we also need to update reference to those stages
        if is_completed {
            let locations = self.stage_partitions.remove(&stage_id).unwrap();

            let locations = locations
                .into_iter()
                .flatten()
                .map(|p| (p.partition_id.partition_id, p))
                .fold(
                    HashMap::new(),
                    |mut state: HashMap<usize, Vec<PartitionLocation>>, (key, p)| {
                        state
                            .entry(key)
            ...
            let l: Vec<Vec<PartitionLocation>> = locations.into_values().collect();

            self.planner.resolve_stage(stage_id, l.clone())?;

            let (runnable, _) = self.planner.actionable_stages()?;
            ...
            if let Some(runnable) = runnable {
                let stages: ballista_core::error::Result<HashMap<usize, ExecutionStage>> =
                    runnable
                        ...
                        .collect();
                let stages = stages?;
                // resolved_stages.extend(stages.keys());

                self.stages.extend(stages);
            } else {
                // There is no more tasks to run
                self.output_locations = l.into_iter().flatten().collect();

                //self.output_locations = vec![];
            }
        }

        // TODO: we need to keep this part as well
        //       in case we need to reply plan
