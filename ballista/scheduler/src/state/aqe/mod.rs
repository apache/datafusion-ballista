...
use datafusion::{
    common::Result, execution::SessionState, physical_plan::ExecutionPlan,
    physical_planner::DefaultPhysicalPlanner,
};
use execution_plan::{AdaptiveDatafusionExec, ExchangeExec};
use log::{debug, warn};
use optimizer_rule::{
    DistributedExchangeRule, EliminateCoalesceBatchesRule, EliminateCooperativeExecRule,
    EliminateEmptyExchangeRule, EliminateRoundRobbinRule, PropagateEmptyExecRule,
...
pub type RunnableStages = Option<Vec<Arc<ShuffleWriterExec>>>;
pub type CancellableStageIds = HashSet<usize>;
...
impl AdaptivePlanner {
    ...
    pub fn try_new_with_optimizers(
        ...
        let session_state =
            Self::create_session(session_config, physical_optimizer_rules);
        ...
        Ok(Self {
            ...
            max_passes: 3, // TODO: read this from config
            job_name,
        })
    }
    ...
    pub fn cancel_stage(&mut self, stage_id: usize) -> Result<()> {
    ...
    /// # Returns
    /// A `Result` indicating success or failure.
    fn replan_stages(&mut self) -> Result<()> {
    ...
    pub(crate) fn runnable_stages(&mut self) -> Result<RunnableStages> {
        let (runnable_stages, _) = self.actionable_stages()?;

        Ok(runnable_stages)
    }
    /// Retrieves the cancellable stages from the current plan.
    ///
    /// # Returns
    /// A `Result` containing the cancellable stage IDs.
    pub(crate) fn cancellable_stages(&mut self) -> Result<CancellableStageIds> {
        let (_, cancellable_stages) = self.actionable_stages()?;
        Ok(cancellable_stages)
    }
    ...
    pub fn actionable_stages(&mut self) -> Result<(RunnableStages, CancellableStageIds)> {
        match self._runnable_stages()? {
            None => {
            ...
            Some(stages) => {
                let (stage_ids, shuffle_writers) = stages
                    ...
                    .collect::<Result<(HashSet<_>, Vec<_>)>>()?;
    ...
    fn default_optimizers() -> Vec<PhysicalOptimizerRuleRef> {
        ...
        physical_optimizers.push(Arc::new(EliminateCoalesceBatchesRule::default()));
    ...
    fn find_runnable_exchanges(
        ...
        mut runnable_stages: &mut Vec<Arc<dyn ExecutionPlan>>,
    ) -> bool {
