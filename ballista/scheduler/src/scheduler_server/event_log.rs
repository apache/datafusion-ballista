...
pub(crate) fn job_end_event(
    ...
    job_end_event_inner(graph, status, queued_at, completed_at, None)
}

/// Shared body of [`job_end_event`] and [`job_cancel_event`].
///
/// `status_override` replaces the job's status strings before anything is
/// serialized. Doing it here rather than rewriting the stored JSON afterwards
/// keeps the payload a faithful serialization of one DTO, which is the property
/// that lets it be relayed verbatim.
fn job_end_event_inner(
    graph: &ExecutionGraphBox,
    status: JobEndStatus,
    queued_at: u64,
    completed_at: u64,
    ...
    let mut job = graph_to_job_response(graph, PlanFormat::Default);
    if let Some(status) = status_override {
        job.status = status.to_string();
        job.job_status = status.to_string();
    }
    ...
    // is deterministic.
    let stages = graph_to_query_stages(graph, PlanFormat::Default, completed_at as u128);
    let dot = build_job_dot(graph).unwrap_or_default();
    let config = session_config_to_job_config(graph.session_config().as_ref());

    // The frozen summary the history server lists jobs from, so it never has to
    // parse the payloads below to render the job list.
    let index = JobIndex {
    ...
    // these REST shapes change. See `ballista_history::event::JobEnd`.
    let job = to_raw(&job);
    let stages = to_raw(&stages);
...
/// Serialize a response for storage. A DTO that cannot be serialized would be
/// a bug rather than a runtime condition, but event logging must never fail a
/// job, so this degrades to an empty object and logs instead of panicking.
fn to_raw<T: serde::Serialize>(value: &T) -> Box<serde_json::value::RawValue> {
    match serde_json::value::to_raw_value(value) {
        Ok(raw) => raw,
        Err(e) => {
            log::warn!("event log: failed to serialize a response for storage: {e}");
...
/// job as running here. The job's status fields are therefore overridden before
/// the record is built, otherwise the history server would show a cancelled job
...
mod tests {
    ...
    async fn job_cancel_event_reports_cancelled_not_running() {
        ...
        let json = serde_json::to_string(&event.to_record().unwrap()).unwrap();
