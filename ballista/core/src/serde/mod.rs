...
use crate::extension::{BallistaCacheNode, SessionConfigExt, SessionConfigHelperExt};
...
use crate::execution_plans::{
    BufferExec, BufferMode, ChaosExec, CoalescePlan, DistributedQueryExec,
    OrderedRangeRepartitionExec, PartitionGroup, PerPartitionFilterExec,
...
impl BallistaLogicalExtensionCodec {
    /// Creates a Ballista logical codec that delegates non-Ballista objects to
    /// `default_codec` while retaining Ballista's file-format codecs.
...
impl LogicalExtensionCodec for BallistaLogicalExtensionCodec {
    ...
    fn try_decode_higher_order_function(
        ...
        name: &str,
        buf: &[u8],
    ) -> Result<Arc<datafusion::logical_expr::HigherOrderUDF>> {
        self.default_codec
            .try_decode_higher_order_function(name, buf)
    }
    ...
    fn try_encode_higher_order_function(
        ...
        self.default_codec
            .try_encode_higher_order_function(node, buf)
    }
    ...
    fn try_decode_udaf(
        ...
        self.default_codec.try_decode_udaf(name, buf)
    }
    ...
    fn try_encode_udaf(
        ...
        self.default_codec.try_encode_udaf(node, buf)
    }
    ...
    fn try_decode_udwf(
        ...
        name: &str,
        buf: &[u8],
    ) -> Result<Arc<datafusion::logical_expr::WindowUDF>> {
        self.default_codec.try_decode_udwf(name, buf)
    }
    ...
    fn try_encode_udwf(
        ...
        self.default_codec.try_encode_udwf(node, buf)
    }
}
...
impl PhysicalExtensionCodec for BallistaPhysicalExtensionCodec {
    fn try_decode(
        ...
        match ballista_plan {
            ...
            PhysicalPlanType::PerPartitionFilter(node) => {
                let [input] = inputs else {
                    return Err(DataFusionError::Internal(format!(
                        "PerPartitionFilterExec expects exactly 1 input, got {}",
                        inputs.len()
                    )));
                };
            ...
            PhysicalPlanType::DistributedQuery(distributed_query) => {
                ...
                let config = SessionConfig::new_with_ballista()
                    .update_from_key_value_pair(&distributed_query.settings)
                    .ballista_config();
                Ok(Arc::new(
    ...
    fn try_encode(
        ...
        if let Some(exec) = node.downcast_ref::<ShuffleWriterExec>() {
            ...
            Ok(())
        } else if let Some(exec) = node.downcast_ref::<PerPartitionFilterExec>() {
            let predicates = exec
                .predicates()
                .iter()
                .map(|p| {
                ...
                .collect::<Result<Vec<_>, DataFusionError>>()?;
            let proto = protobuf::BallistaPhysicalPlanNode {
                physical_plan_type: Some(PhysicalPlanType::PerPartitionFilter(
                    protobuf::PerPartitionFilterExecNode { predicates },
                )),
            };
            proto.encode(buf).map_err(|e| {
                DataFusionError::Internal(format!(
...
mod test {
    ...
    impl LogicalExtensionCodec for MarkerLogicalCodec {
        fn try_decode(
            ...
            Err(DataFusionError::Plan("marker-plan-decode".to_string()))
        }

        fn try_encode(&self, _node: &Extension, _buf: &mut Vec<u8>) -> Result<()> {
            Err(DataFusionError::Plan("marker-plan-encode".to_string()))
        }
        ...
        fn try_decode_table_provider(
            ...
            Err(DataFusionError::Plan("marker-table-decode".to_string()))
        }

        fn try_encode_table_provider(
            &self,
            _table_ref: &datafusion::common::TableReference,
            _node: Arc<dyn datafusion::catalog::TableProvider>,
            _buf: &mut Vec<u8>,
        ) -> Result<()> {
            Err(DataFusionError::Plan("marker-table-encode".to_string()))
        }
        ...
        fn try_decode_udf(
            ...
            Err(DataFusionError::Plan("marker-scalar".to_string()))
        }
        ...
        fn try_decode_higher_order_function(
            ...
            Err(DataFusionError::Plan("marker-higher-order".to_string()))
        }
        ...
        fn try_decode_udaf(
            ...
            Err(DataFusionError::Plan("marker-aggregate".to_string()))
        }
        ...
        fn try_decode_udwf(
            ...
            Err(DataFusionError::Plan("marker-window".to_string()))
        }
    }

    #[test]
    fn logical_codec_delegates_function_decoding() {
        let codec = BallistaLogicalExtensionCodec::new(Arc::new(MarkerLogicalCodec));
        ...
        assert!(
            codec
                ...
                .unwrap_err()
                .to_string()
                .contains("marker-scalar")
        );
        assert!(
            codec
                .try_decode_udaf("udaf", &[])
                .unwrap_err()
                .to_string()
                .contains("marker-aggregate")
        );
        assert!(
            codec
                .try_decode_udwf("udwf", &[])
                .unwrap_err()
                .to_string()
                .contains("marker-window")
        );
        assert!(
            codec
                .try_decode_higher_order_function("higher", &[])
                .unwrap_err()
                .to_string()
                .contains("marker-higher-order")
        );
    }
    ...
    async fn test_distributed_query_exec_roundtrip() {
        ...
        let mut buf = Vec::new();
        codec
            .try_encode(Arc::new(original_exec), &mut buf, &converter)
            .unwrap();

        let ctx = SessionContext::new().task_ctx();
        let decoded_plan = codec.try_decode(&buf, &[], &ctx, &converter).unwrap();
    ...
    /// `PerPartitionFilterExec` round-trips through the codec: three input
    /// partitions with three distinct range predicates re-materialize
    /// on the other side with the same predicate strings in the same
    /// order.
    #[tokio::test]
    async fn test_per_partition_filter_exec_roundtrip() {
        use crate::execution_plans::PerPartitionFilterExec;
        ...
        let range = |lo: i64, hi: i64| -> Arc<dyn PhysicalExpr> {
            let c: Arc<dyn PhysicalExpr> = Arc::new(Column::new("v", 0));
            let ge: Arc<dyn PhysicalExpr> = Arc::new(BinaryExpr::new(
                c.clone(),
                Operator::GtEq,
                Arc::new(Literal::new(ScalarValue::Int64(Some(lo)))),
            ));
            let lt: Arc<dyn PhysicalExpr> = Arc::new(BinaryExpr::new(
                c,
                Operator::Lt,
                Arc::new(Literal::new(ScalarValue::Int64(Some(hi)))),
            ));
            Arc::new(BinaryExpr::new(ge, Operator::And, lt))
        };
        let predicates = vec![range(0, 10), range(10, 20), range(20, 30)];
        let original =
            PerPartitionFilterExec::try_new(input.clone(), predicates.clone()).unwrap();
        ...
        assert_eq!(decoded.predicates().len(), 3);
        for (k, expected) in predicates.iter().enumerate() {
            assert_eq!(
                decoded.predicates()[k].to_string(),
                expected.to_string(),
