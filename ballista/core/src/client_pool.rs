...
//! `(host, port)` key backed by a [`DashMap`]. Callers [`acquire`] a
//! [`PooledClient`] guard; when the guard is dropped the underlying client is
//! returned to the idle deque automatically. If the connection errored, call
//! [`PooledClient::discard`] before dropping so the pool closes the channel
