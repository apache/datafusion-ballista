...
struct InputPartitionOutput {
    ...
    encoded: Vec<Vec<u8>>,
    /// `(num_batches, num_rows, num_bytes)` for the rows in `encoded`.
    in_memory_stats: Vec<(u64, u64, u64)>,
...
fn encode_buffered_partitions(
...
) -> Result<(Vec<Vec<u8>>, Vec<(u64, u64, u64)>)> {
    ...
    for partition_indices in indices.iter() {
        ...
        encoded.push(buf);
        stats.push((num_batches, num_rows, num_bytes));
    }
...
fn write_task_consolidated(
    ...
    for partition_id in 0..num_partitions {
        ...
        for out in outputs.iter() {
            ...
            let (mem_batches, mem_rows, mem_bytes) = out.in_memory_stats[partition_id];
            num_batches += mem_batches;
            num_rows += mem_rows;
            num_bytes += mem_bytes;
        }
...
async fn run_coordinator(
    ...
    let mut outputs: Vec<InputPartitionOutput> =
        Vec::with_capacity(num_input_partitions);
