...
opaque identity in scheduler state and history event logs. It is intentionally
separate from the scheduler callback endpoint built from `external_host` and
`bind_port`. Code that constructs `SchedulerConfig` with a struct literal must
provide this field. Prefer the existing constructors/builders where possible;
use `SchedulerConfig::with_scheduler_id(...)` when an embedding needs a stable
scheduler id.
