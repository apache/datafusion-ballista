...
| ballista.shuffle.merge_ordered_passthrough               | Boolean | true      | When a passthrough shuffle stage's input is sorted, merge the task's partitions into one sorted output partition instead of writing one file per partition. The consumer plants an ordering preserving reader that merges anyway, so this moves that merge into the producing task and cuts the consumer's fan-in from the stage's partition count to its task count. Set to false to write one file per partition.                                                                                                           |
