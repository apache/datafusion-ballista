...
| ballista.shuffle.sort_based.enabled                | Boolean | false       | Enables the sort-based shuffle writer (consolidated data file per input partition with an index, instead of one file per output partition).  |
