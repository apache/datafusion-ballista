...
use kapot::prelude::{KapotContext, KapotError, Result};
use clap::ValueEnum;
...
impl Command {
    pub async fn execute(
        ...
        match self {
            ...
            Self::Quit => Err(KapotError::Internal(
                "Unexpected quit, this should be handled outside".to_string(),
            )),
            Self::ListFunctions => {
                display_all_functions().map_err(KapotError::DataFusionError)
            }
            Self::SearchFunctions(function) => {
                if let Ok(func) = function.parse::<Function>() {
                    ...
                    Err(KapotError::NotImplemented(msg))
                }
            }
            Self::OutputFormat(_) => Err(KapotError::Internal(
